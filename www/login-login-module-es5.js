...
(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["login-login-module"], {
  ...
  function srcAppLoginLoginPageTs(module, __webpack_exports__, __webpack_require__) {
    ...
    /*! firebase/auth */
    "./node_modules/firebase/auth/dist/index.esm.js"); // import { Storage } from '@ionic/storage';


    var LoginPage =
    /*#__PURE__*/
    function () {
      function LoginPage(router, menu, toast, store, platform) {
      ...
      _createClass(LoginPage, [{
        ...
        value: function doLogin() {
          ...
          this.isLoading = true;
          console.log('do Log In'); //     firebase.auth().currentUser.

          firebase_app__WEBPACK_IMPORTED_MODULE_7__["auth"]().signInWithEmailAndPassword(this.loginForm.get('email').value, this.loginForm.get('password').value).then(function (res) {
            console.log('login successful');

            _this.store.setItem('hoa-uid', res.user.uid); // if (this.platform.is('android') || this.platform.is('ios')) {
            ...
            if (firebase_app__WEBPACK_IMPORTED_MODULE_7__["database"]().ref('resident').child(res.user.uid).once('value') !== null) {
              console.log('isIT', firebase_app__WEBPACK_IMPORTED_MODULE_7__["database"]().ref('resident').child(res.user.uid).once('value', function (snap) {
                return snap.val();
              }));
              firebase_app__WEBPACK_IMPORTED_MODULE_7__["database"]().ref('resident').child(res.user.uid).once('value', function (snapshot) {
                _this.store.setItem('hoa-role', snapshot.val().role); // for (const key in snapshot.val()) {
                ...
                _this.toast.showToast("Welcome Back ".concat(snapshot.val().fullname.split(' ')[0]), '2000', 'bottom'); // } else {
            ...
            if (firebase_app__WEBPACK_IMPORTED_MODULE_7__["database"]().ref('security').child(res.user.uid).once('value') !== null) {
              firebase_app__WEBPACK_IMPORTED_MODULE_7__["database"]().ref('security').child(res.user.uid).once('value', function (snapshot) {
                // for (const key in snapshot.val()) {
                //       if (snapshot.val().hasOwnProperty(key)) {
                //             const element = snapshot.val()[key];
                //             if (key === res.user.uid) {
                // window.localStorage.setItem('hoa-role', snapshot.val().role);
                _this.store.setItem('hoa-role', snapshot.val().role); // if (this.platform.is('android') || this.platform.is('ios')) {
                //       window.localStorage.setItem('hoa-role', snapshot.val().role); // REMOVE BEFORE BUILD
                //       this.store.setItem('hoa-role', snapshot.val().role).then(() => {
                //             console.log('ROLE SAVED');
                //       }).catch((err: Error) => {
            ...
            if (firebase_app__WEBPACK_IMPORTED_MODULE_7__["database"]().ref('dependent').child(res.user.uid).once('value') !== null) {
              firebase_app__WEBPACK_IMPORTED_MODULE_7__["database"]().ref('dependent').child(res.user.uid).once('value', function (snapshot) {
                // for (const key in snapshot.val()) {
                //       if (snapshot.val().hasOwnProperty(key)) {
                //             const element = snapshot.val()[key];
                //             if (key === res.user.uid) {
            ...
            _this.toast.showToast("".concat(err.message), '4000', 'bottom');
    ...
    }), tslib__WEBPACK_IMPORTED_MODULE_0__["__metadata"]("design:paramtypes", [_angular_router__WEBPACK_IMPORTED_MODULE_3__["Router"], _ionic_angular__WEBPACK_IMPORTED_MODULE_4__["MenuController"], _toast_service__WEBPACK_IMPORTED_MODULE_6__["ToastService"], _hoa_store_service__WEBPACK_IMPORTED_MODULE_5__["HoaStoreService"], _ionic_angular__WEBPACK_IMPORTED_MODULE_4__["Platform"]])], LoginPage);
    /***/
  },
